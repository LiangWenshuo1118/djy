...
其中，$\tau_j$和$\Theta_j$ 是分子$j$的平移和旋转，而 $\xi_i$ 代表原子$i$在分子$j$参考系中的内部坐标。我们将模拟盒子拆解为单个分子。

对于每一个分子，我们保持每个分子的原始几何形态 $\xi_i$，并重新计算其所有的属性，得到整个配置的纯分子内部贡献：

\begin{equation}
\begin{aligned}
& E^{\text {intra }}\left(\tau_j+\Theta_j \xi_i\right)=\sum_j E_j^{\text {intra }}\left(\xi_i\right) \\
& F^{\text {intra }}\left(\tau_j+\Theta_j \xi_i\right)=\bigcup_j \Theta_j^T F_j^{\text {intra }}\left(\xi_i\right)
\end{aligned}
\end{equation}

其中，分子内能对 $\tau_j$ 和 $\Theta_j$ 保持不变，而分子内力和应力张量对 $\tau_j$ 保持不变，并与 $\Theta_j$ 等效。我们计算分子间贡献为差值：
